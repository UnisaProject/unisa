...
<head>
  ...
  <style type="text/css">
    ...
    $(document).ready(function(){  
    	...
    	var myNetID = $('#netID').val();
    	var myMacCode = $('#macCode').val();
    	//alert("Result NetworkID="+myNetID+", MACCODE="+myMacCode);
    	
    	// open the PDF in a new window
    	$('#printPDFButton').click(function () {
    		//alert("Print PDF Button");
    		var reportErr = false;
    		var url = 'tracking.do?act=reportPDF&NETWORKID='+myNetID;
    		//alert("Ajax Step 1 - URL="+url);
        ...
        $('#closeButton').click(function () {
        	...
        	//Check if client is myUnisa or LSS GUI
			var guiClient = $('#isLSSClient').val();
			//alert("Is this a GUI Client="+guiClient);
			if (bName === "IE" && String(guiClient).toLowerCase() === 'false'){
				var url="tracking.do";
				window.parent.opener.location.href=url;
        		window.close();
        	}else{
	        	var url="tracking.do?NETWORKID="+myNetID+"&MACCODE="+myMacCode;
    	...
    	function setAction() {
    	  	var myNetID = $('#netID').val();
    		document.trackingForm.action = 'tracking.do?act=report&NETWORKID='+myNetID;
...
<div id="content" class="page">
														...
														<td style="width:50%" align="right">
															<bean:define id="docID" name="listOfRecords" property="docketNumber"/>	
																<%if (!docCompare.toString().equalsIgnoreCase(docID.toString())){ %>
																	...
																	<bean:define id="docCompare2" name="listOfRecords" property="docketNumber"/>
																<%}else{ %>
																	&nbsp;
																<%} %>
														</td>
	...
	<div class="noPrint">
		<div class="panel panel-default">
		  	<div class="panel-body" style="padding-top:0px !important; padding-bottom:0px !important">
  				<input class="btn btn-default" type="button" id="closeButton" value="CLOSE" />
	  			&nbsp;<input class="btn btn-default" type="button" id="printButton" value="PRINT" />
	  			&nbsp;<a class="btn btn-default" href="tracking.do?act=reportPDF" role="button">GENERATE PDF</a>
