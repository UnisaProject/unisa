...
import za.ac.unisa.lms.tools.studentregistration.forms.FileBean;
...
public class UploadAction extends DispatchAction {
	...
	public ActionForward uploadInput(ActionMapping mapping,
		...
		if(stuRegForm.getStudent().getNumber() == null || "".equals(stuRegForm.getStudent().getNumber())){
			log.info("UploadAction - uploadInput - Student Nr is null or empty when saving qualifications");
		...
		log.debug("UploadAction uploadInput - " + ((codeSource != null)? codeSource.getLocation():"no code source for upload" ));
	...
	public ActionForward upload(ActionMapping mapping,
		...
		// if student number is null, block application
		if(stuRegForm.getStudent().getNumber() == null || "".equals(stuRegForm.getStudent().getNumber())){
			log.debug("UploadAction - Upload - Student Nr is null or empty when saving qualifications");
			messages.add(ActionMessages.GLOBAL_MESSAGE,
			new ActionMessage("message.generalmessage", "You performed and invalid action or an Error occurred. Please log on again to retry."));
			addErrors(request, messages);
			if (stuRegForm.getAdminStaff().isAdmin()){
		...
		if(request.getParameter("Logout") != null) {
			...
			resetForm((StudentRegistrationForm) stuRegForm, "Upload1");
		...
		try{
			...
			if(request.getParameter("Continue") != null) {
				...
				ApplyForStudentNumberQueryDAO applyDAO = new ApplyForStudentNumberQueryDAO();
				...
				log.debug("UploadAction stuRegForm stuExist: " + stuRegForm.getStudent().getStuExist());
				log.debug("UploadAction request Parameter stuExist: " + request.getParameter("stuExist"));
				
				if ("curStu".equalsIgnoreCase(stuRegForm.getStudent().getStuExist()) || "curStu".equalsIgnoreCase(stuRegForm.getStudent().getStuExist()) || "curStu".equalsIgnoreCase(request.getParameter("stuExist")) ){
					...
					resetForm((StudentRegistrationForm) stuRegForm, "Upload Return");
					...
					log.debug("UploadAction - New Student");
					String checkMD = dao.getFinalMD(stuRegForm.getStudent().getNumber(),stuRegForm.getStudent().getAcademicYear(),stuRegForm.getStudent().getAcademicPeriod());
					
					log.debug("UploadAction - checkMD : " + checkMD);
					log.debug("UploadAction - LoginSelectMain : " + stuRegForm.getLoginSelectMain());
					
					if ("MD".equalsIgnoreCase(stuRegForm.getLoginSelectMain()) || "M".equalsIgnoreCase(checkMD) || "D".equalsIgnoreCase(checkMD)){
						log.debug("UploadAction - New Student - MD");
						//Get saved qualifications for student from STUAPQ (Getting them individually to simplify moving result between forms)
						...
						boolean isAdmin = stuRegForm.getAdminStaff().isAdmin();
						//Reset All Form Variables
						resetForm((StudentRegistrationForm) stuRegForm, "Upload MD");
						//Set Necessary variables for Final page
						stuRegForm.getStudent().setAppTime(displayDate);
						...
						stuRegForm.getAdminStaff().setAdmin(isAdmin);
						log.debug("UploadAction - stuExist Redirect: NewMD");
						return mapping.findForward("submitNewMD");
					}else {
						log.debug("UploadAction - New Student - Non MD");
						//Get saved qualifications for student from STUAPQ (Getting them individually to simplify moving result between forms)
						String studentNr = stuRegForm.getStudent().getNumber();
						...
						boolean isAdmin = stuRegForm.getAdminStaff().isAdmin();
						//Reset All Form Variables
						resetForm((StudentRegistrationForm) stuRegForm, "Upload New");
						//Set Necessary variables for Final page
						stuRegForm.getStudent().setAppTime(displayDate);
						stuRegForm.getStudent().setNumber(studentNr);
	 ...
	 private String uploadFile(String filePath,InputStream is) {
		...
		**/
		public String moveDocuments(StudentRegistrationForm stuRegForm) {
	...
	public void resetForm(StudentRegistrationForm form, String callAction)	throws Exception {
	...
	public void reLoad(StudentRegistrationForm form) throws Exception{
		form.getDesc().clear();
		form.getMap().clear();
		SavedDocDao savedDocDao = new SavedDocDao();
		log.debug("StudentRegistrationForm - reLoad - Reloading uploaded docs..");
		savedDocDao.getAllNonRequiredDocInfo(form.getDesc(), form.getMap(), form.getStudent().getNumber(),form.getStudent().getAcademicYear(),form.getStudent().getAcademicPeriod(),form.getStudent().getStuExist(),form.getStudent().getMatrix());
